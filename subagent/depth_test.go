...
func newTestAgent(t *testing.T, registry *agent.AgentRegistry, id string, options agent.AgentOptions, headerDepth *int64) *agent.Agent {
	t.Helper()
	sess, err := session.NewDetached(session.SessionID(id), nil, &session.SessionHeader{Version: session.SESSION_FORMAT_VERSION, ID: session.SessionID(id), DelegationDepth: headerDepth}, 0)
	if err != nil {
