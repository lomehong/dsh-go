...
func TestSeedDescriptorTurn(t *testing.T) {
	...
	prefix, err := session.NewDetached(session.SessionID("child-1"), nil, &session.SessionHeader{Version: session.SESSION_FORMAT_VERSION, ID: session.SessionID("child-1")}, 0)
	if err != nil {
