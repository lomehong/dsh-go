...
- **R28（policy/skill/deepseek-extensions 批组装 + subprocess 行名对齐）**
- 落地面：`boot/catalog.go` 七条目——`@deepseek-ai/dsh-tool-call-timeout-policy`（guard.AttachTimeoutPolicy 挂工具管线）、`@deepseek-ai/dsh-agent-instructions`（maxBytes 缺省 65536、DSHHome=profile home）、`@deepseek-ai/dsh-skill-filesystem`（ResolveConfig 默认齐备、RegisterProviderIn create 回调接 control.Invalidate）、`@deepseek-ai/dsh-skill-badge`（嵌入资产物化到 home/skill-badge）、`@deepseek-ai/dsh-session-checkpoint-policy`（Inject 五服务、checkpointFlusherOf 适配 Coordinator.FlushSession）、`@deepseek-ai/dsh-deepseek-llm-api-extensions`（Provide deepseekLlmApiExtensions）、`@deepseek-ai/dsh-session-log-deepseek`（enabled 缺省 false 不注册）；`llm/deepseek/plugin.go` PluginDeps.Extensions 缝 → AdapterOptions.Extensions；llm-deepseek 条目机会式消费扩展注册表；subprocess 条目更名 dsh-subprocess-local 对齐官方 bundle 行名（此前名实不符会让官方 profile loud-miss）。
- 测试：`boot/catalog_test.go` +1 TestCatalogPolicySkillAndExtensionsBatch——十三件装配→扩展注册表 Prepare 无错→skill-badge 资产目录物化断言→Shutdown 干净；subprocess 更名处既有组装测同步。
- 决策记录（README 语义决策新两条）：deepseek 扩展注册表=可选伴件服务（ctx.get 同形、组合序静态化）；checkpoint Flusher=id→live 会话适配、查无会话=拒绝检查点保守阻止分发。
