...
func TestSubagentStartInjectsChildContextAndPairedEndRuns(t *testing.T) {
	...
	childSession, err := session.NewDetached(session.SessionID("child-1"), nil, &session.SessionHeader{Version: session.SESSION_FORMAT_VERSION, ID: session.SessionID("child-1"), CWD: f.dir}, 0)
	if err != nil {
