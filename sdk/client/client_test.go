...
func (f *stubAgents) Create(sessionID string, options server.CreateAgentOptions) (*agent.Agent, error) {
	...
	sess, err := session.NewDetached(session.SessionID(sessionID), nil, &session.SessionHeader{Version: session.SESSION_FORMAT_VERSION, ID: session.SessionID(sessionID), CWD: options.Cwd}, 0)
	if err != nil {
		return nil, err
	}
