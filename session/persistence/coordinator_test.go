...
func TestCoordinatorWritePathBatchesAndFlushes(t *testing.T) {
	backend := newMemoryBackend()
	coordinator := newTestCoordinator(t, backend)
	header := testHeader("live1")
	live, err := session.NewDetached("live1", nil, &header, 0)
	if err != nil {
...
func TestCoordinatorOnCreatedRejectsSeedCollision(t *testing.T) {
	...
	collision := testHeader(id)
	live, err := session.NewDetached(id, []session.Event{mustEvent(t, session.EventUserMessage, 0, validUserMessageData("different"))}, &collision, 0)
	if err != nil {
		t.Fatalf("detached: %v", err)
	}
	coordinator.AttachLive(live)
	...
	live2, err := session.NewDetached(id2, seed, func(h session.SessionHeader) *session.SessionHeader { return &h }(testHeader(id2)), 0)
	if err != nil {
