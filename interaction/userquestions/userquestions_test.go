...
func newTestService(t *testing.T) (*Service, *agent.AgentRegistry, *agent.Agent) {
	...
	sess, err := session.NewDetached(session.SessionID("asker"), nil, &session.SessionHeader{Version: session.SESSION_FORMAT_VERSION, ID: session.SessionID("asker")}, 0)
	if err != nil {
...
func TestLiveCallerCheck(t *testing.T) {
	...
	ghostSession, err := session.NewDetached(session.SessionID("ghost"), nil, &session.SessionHeader{Version: session.SESSION_FORMAT_VERSION, ID: session.SessionID("ghost")}, 0)
	if err != nil {
		t.Fatalf("NewDetached: %v", err)
	}
...
func TestOwnedCallerCannotAsk(t *testing.T) {
	...
	childSession, err := session.NewDetached(session.SessionID("child"), nil, &session.SessionHeader{Version: session.SESSION_FORMAT_VERSION, ID: session.SessionID("child")}, 0)
	if err != nil {
		t.Fatalf("NewDetached: %v", err)
	}
