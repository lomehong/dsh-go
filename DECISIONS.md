...
- **code-runtime-worker-thread 决策（web 模式 PTC 代码执行延后）**：官方 @deepseek-ai/dsh-code-runtime-worker-thread 是 web 模式 PTC run_code 的代码执行运行时（worker_threads 承载）。Go 宿主无进程内 JS 引擎，且 coderuntime seam 已移植但传输/后端依赖 JS 基底决策（ROADMAP 受阻盘点）。本轮处置为 T3-planned-skip——组合跳过+warn「web-mode PTC code execution deferred」；**web UI 的代码执行功能缺席**（模型可运行 Go 原生 workflow，JS run_code 不可用），如实注记。
- **session-reference 错误韧性 Go 裁决（偏差入账）**：官方 pre-step 监听器 PrepareDirectMessages 失败时 throw → 响亮 turn 失败。Go 宿主裁决为**韧性优先**：单个坏引用降级 next()（日志 Warn）而不炸整个 step——偏差已显式入账（boot/catalog.go 注释 + ROADMAP r139 r 段），非隐性分歧。
- **gojaruntime = goja 纯 Go ES2015+ JS 引擎（PTC run_code 执行基底）**：官方 code-runtime-worker-thread 依赖 Node worker_threads，Go 无此基底。选型 goja（github.com/dop251/goja，纯 Go 无 CGO）：每 Run 新 VM + host bindings 挂全局 + goja.Interrupt 硬超时。Language 报告 "javascript"（非 "typescript"——goja 无 TS 语法转译）。async 体以 (async function(){...})() 包装，Promise settled 结果经 State() 判定后取 Result()——async throw 不静默吞（rejected → FailureException）。Isolation 报告 "in-process-goja"（非 sandbox 隔离）。
