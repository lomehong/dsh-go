...
func (f *fixture) startAgent() {
	f.t.Helper()
	sess, err := session.NewDetached(session.SessionID("codex-agent"), nil, &session.SessionHeader{Version: session.SESSION_FORMAT_VERSION, ID: session.SessionID("codex-agent"), CWD: f.dir}, 0)
	if err != nil {
