...
func TestPreStepArmFlushesAgentSession(t *testing.T) {
	...
	defer detach()
	sess, err := session.NewDetached(session.SessionID("checkpoint-1"), nil, &session.SessionHeader{Version: session.SESSION_FORMAT_VERSION, ID: session.SessionID("checkpoint-1"), CWD: "D:\\tmp"}, 0)
	if err != nil {
