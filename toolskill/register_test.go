...
func newRig(t *testing.T, config Config) *testRig {
	...
	sess, err := session.NewDetached(session.SessionID("agent-1"), nil, &session.SessionHeader{Version: session.SESSION_FORMAT_VERSION, ID: session.SessionID("agent-1"), CWD: "D:\\proj"}, 0)
	if err != nil {
