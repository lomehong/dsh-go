...
func TestCompactionEventsRegistered(t *testing.T) {
	...
	sess, err := session.NewDetached("c1", nil, &session.SessionHeader{Version: session.SESSION_FORMAT_VERSION, ID: "c1"}, 0)
	if err != nil {
...
func newSessionWithSurface(t *testing.T, id string) (*session.Session, *agent.Agent) {
	t.Helper()
	sess, err := session.NewDetached(session.SessionID(id), nil, &session.SessionHeader{Version: session.SESSION_FORMAT_VERSION, ID: session.SessionID(id)}, 0)
	if err != nil {
		t.Fatalf("NewDetached: %v", err)
	}
