...
**剩余（按依赖序；下方"70/86"等为 r48 时点历史快照，当前权威计数见 STATUS 生成区块 + boot/dispositions.go + 组合守卫）**：
...
### 剩余面权威清单（**历史快照**：r39 程序化审计 + r48 深盘定谳；**r139 账实复审后本节不再反映现状**——goal 族 5 行、session-telemetry-otel、tool-session-query、llm-retry 均已接线；llm-pi-ai 的"处置不移植"结论已被 r135-r138 推翻（pi-ai 孪生行 + OpenAI/Anthropic wire 落地）；timer 已挂 no-op stub 行。当前权威 = STATUS 生成区块 + boot/dispositions.go + 组合守卫）
...
### 剩余面依赖序地图（r37 盘点，实证自官方源；**历史盘点快照——web 家族/title 族/goal 族/session-query/sqlite/telemetry/llm-retry 等已在后续轮清账，未清项以 r139 复审注与 dispositions 表为准**）

- **cmd/dsh web serve 落地（r67，2026-09-01）**：`dsh web` 别名（= `--profile web`，官方硬别名逐字）+ `--host`/`--port`（默认 127.0.0.1:3080 对齐官方 dsh-web-app bundle fallback）+ `host/webhost` 包（frontend fallback owner：index.html 经 registry 注入渲染 + dist 静态资源 + 无扩展路径 SPA 回落、路径遍历封堵；`ResolveFrontendDist` 沿安装锚上溯解析 `node_modules/@deepseek-ai/dsh-frontend/dist`，缺失 fail-loud "frontend dist not built" 官方 build 提示逐字；listen 生命周期归 Go http.Server 消费方——STATUS 已记录的 Go 惯例决策）。端到端 smoke 实测：web profile 组合 → 绑定 → URL 打印 → `GET /` 200（含 `__DSH_BOOT_READY__` 注入）→ 静态资源 → SPA fallback → 优雅关停，全绿。**如实记录**：完整 web 组合依赖 npm 官方包可安装（当前 rc 阶段 peer 依赖 `dsh-bash-env` 等 404）与 catalog 覆盖 web-app 行（`dsh-web-app`/startup、`dsh-host-apiproxy`/api-gateway、client 家族、agent-presets 等未接线）——serve 路径已落地，浏览器会话功能面随 alpha.3 B 系列轮。

- **alpha.3 B 系列进度（红队对齐简报，上游 dd6322d）**：B1 projection identity-gated change feed ✅（r69，穷举 960 场景钉住）；B2 gatewaystream stalled hosts ✅（r70+r71，心跳基线补移植 + r61/62 潜伏 bug 修复 + 测试确定性加固）；B3 closing-turn wake tracking **✅ 无操作**——git 实证：alpha.2→alpha.3 agent.ts 零 diff，收窄提交 2c1cc3e778 经后续合并被解析抵消未存活到 tag（**观察项：存活于上游 master，预计 alpha.4 基线升级轮重新打开**）；B4 subagent image follow-up admission ✅（r72，MODEL_DOES_NOT_SUPPORT_IMAGES 门 + 同步模型无 await-recheck 适配）；B5 图片链后半待续；B6 prompt admission ✅（AdmitPromptContent 已随 attachment 批落地）；B7 read_image 无扩展名 ✅（sniffImageMediaType 已随 read_image 批落地）；steer-followup 可靠送达待续。
...
| r127 | 验证轮（三项逐一对照上游修复语义，全为已满足）：agent terminate live attempts（a65edc4810 的 legacyChunkSeq 框架是 v1 专属——Go r122 的 v2 设计即其 HEAD 后继：取消→持久 attempt settlement、append 失败→abandoned end）；cold listing body-free（17f49f6191）——Go session/list 走 corpus ListSessions（仅 header）+ identity 校验的 cache hint，从不打开冷 body；surface 端点/compaction span（b5f1539733/f9870325b2）——v01 验证已按修复后 `start>=seq||end>=seq` 移植（端点序自由、位置序由运行时 fold 的 indexOf 管辖），核心 replacementRange 的 startIdx>endIdx 拒绝与上游 HEAD 逐字一致 | ✅（无代码变更） |
| r128 | jsonl 跨进程写权租约（c58097a826）：内核仲裁锁——POSIX flock(LOCK_EX\|LOCK_NB)+inode 复验三试（锁文件永不删除保 inode 稳定）；Windows 命名信号量（kernel32 LazyProc 直绑 CreateSemaphoreW/ReleaseSemaphore+ WaitForSingleObject(0)，路径规范化哈希派生名——无锁文件，读者/搜索/目录删除不受阻）；竞争映射 persistence.AlreadyOwnedError（官方 SessionAlreadyOwnedError 文案）；进程死亡内核自动释放、活僵持无过期（防撕裂追加）；Backend 在 write-open/首物化写/CommitRepair 前获取、per-dir 缓存持有、Close 全释放；双进程测试（外来持有者拒绝→释放→追加成功） | ✅ |
| r129 | session-stats 接线 + session-log-export 处置收窄 + tokenmeter file 定价缝 | ✅ |
| r130 | store→cordis 事件桥（真实缺陷修复）+ session/follow live 续流实装 | ✅ |
| r131 | headless 档案补全 + 三真实缺陷修复（ReasoningEffort 丢失/ResolveUserID nil/凭证缺环境层）；**遗留：流中段 context canceled 需异步 run 专轮** | ✅（遗留项入待续） |
| r132 | 浏览器打开时机修复——挂载完成后移交，消除首屏 404 竞态 | ✅ |
| r133 | settings 写端点返回 namespace view——修复声明保存失败与 mirror null 崩溃 | ✅ |
| r134 | 预设目录随包暂存 + directory-picker 双面跳过——设置页三 Tab 数据修复 | ✅ |
| r135 | 插件行启停语义修正 + Models 页目录提供方解锁 | ✅ |
...
| r139 | 审计修复轮（Owner 授权系统推进，a-g 七提交）：**a** 门禁可信——webhost MIME 断言改平台注册表对齐（test-rot）、守卫 v5 补 clone 新路径候选、guard 名册逐字同步上游 base+web-app（删自加 report 行、修 fetch 漂移与注释乱码）、genstatus 分母改程序化名册事实（85 行/84 唯一名）；**b** 账实复审清账——FINDINGS R7/R9、ROADMAP r48 清单标历史快照 + B6/B7 落地注 + r129-r138 补账、STATUS 过期行修正、处置/头注释如实化；**c** anthropic 投影 image/file/未知块 fail-loud（修 r137 静默丢图）+ R7 残余修复（commands 行挂生产 image admitter）+ acp/sdk/sdk-minimal 16 行处置（三档案从 import 响亮失败变带警组合）；**d** web 聊天回路核心——session.prompt（时区规范校验/回声去重/路由服务门/图片模态门与共享准入/steer-followup 分派）+ session.cancel（user cause + keepInbox）wire 注册 + 10 测；**e** $events/result 闭环——ResultRouter 关联 pending waterfall 与浏览器 unary 应答、桥的阻塞式应答 listener（approval/request 与 user-questions/request 类型化投影 + 结论映射；无 client 委托不打结、请求 signal 取消推 cancel 帧）+ 12 新测；**f** 文件上传 wire——fileUploads 服务（uploadStream 语义：活会话门 + 逐字存储 + 进程内 receipt 授权令牌绑定接收会话）+ POST /api/session/uploadFileBinary（octet-stream/405/415/400 契约逐字）+ fileUploads.upload base64 Remote 兜底 + fileReferences.list @ 补全命名空间 + prompt file 部分走 receipt 解析进持久 file 块（resolve 权限栅栏：外来会话拒）+ 6 新测；**g** commands 控制器——commands.list（会话域目录，name 序 + input 投影）+ commands.execute（admission miss = 官方 value-undefined 语义改述为 arguments-invalid + 官方文案；handler error result 温和结算、handler throw 响亮传播的语义分界保真）+ commands/change 从私有监听表桥上 agent 总线（`$events` 白名单可达，浏览器目录失效闭环）+ 5 新测；**h** selectModel + 投影/标题出口——session/selectModel（路由校验 + SessionSelections 覆盖注册表经 Create 安装的 request waterfall 改写下一次模型调用路由；进程内注册表为已记录偏差至投影轮）+ session/openWorkspacePath（sessionId 作用域时 cwd 词法栅栏 + 平台原生打开器 best-effort）+ session/follow 快照 projections.values 填充（注册表全 wire 单元折叠）+ session/list 标题（活会话经 session-title 快照）；5 新测 ；**j** 通知缝补齐——llm.Runtime.OnAdaptersUpdated（register/replace/dispose 三处触发、锁外包含派发）+ credentials.Provider 接口导出 Notifier；llm/adapters-updated 与 credentials/reference-updated 两事件组合接线收尾（Models 页凭据/适配器实时刷新）；**k** headless 异步 run——RunDeps 急切解析（goroutine 不再与后续行 apply 竞速 ctx.Get）+ StartRun 独立 goroutine 执行任务（同步 Apply 曾阻塞整个组合生命周期并与驱动流中段交互——r131 诊断的 context canceled 根因形态移除）+ 组合提供 headlessRunDone 通道 + launcher 等待完成或中断后 Shutdown/Exit；实机冒烟：组合 0 警告→异步 run 执行→干净退出 CODE=1（无凭据预期路径）；**l** settings 消费闭环——agent-loop 节真 schema（defaults 10 + 正整数 validate 拒写）+ 初始值经 defaults→base→user 折叠进 AgentLoopConfig + OnUpdated 活回灌经 SetMaxParallelToolCalls（atomic 帽，下一工具组生效、在飞组不受扰——官方 getter read-through 语义）+ agent-loop/config-start-failed cordis 事件发射（reportConfiguredStartupFailure 双通道：logger + 总线）；**m** session/search + updateQueue——search 经 sessionquery 引擎跨会话字面量检索（SearchDisabled 稳定码透传、{sessionId,snippet} 官方投影）+ updateQueue 三动作（edit 仅文本/QUEUE_EDIT_NON_TEXT 拒绝、remove 摘除、steer 仅运行中 next-turn 否则 session/steer-unavailable 且保留队列项；定位跨 next-turn/next-step 双 lane 按 message id；cold 会话 = queue-item-not-found）+ 5 新测；fork 因 Go 侧持久化 Fork 域未移植如实保持开放（独立轮）；**n** session/fork——forkBoundary 边界语义（最后完成回合收尾或 atSeq 所含回合、尾随回合间事件随行、无完成回合 fork-unavailable）+ Store.Fork 校验链 + 子会话经 agents.Create 种子化（Meta 血统 ParentSession/IsSeeded/InheritedEventCount + 源 preset 挂载与选路覆盖安装）+ api-session/added 随 fork 触发 + fake factory 透传种子对齐生产 + 4 新测 ；**o** fssearch spill 恢复面落地——SpillSink（spill.Store 窄缝 + exec 属主解析）经 Register 注入；glob/grep 超帽时完整结果持久化并以 savedTo 进 canonical 值（输出 schema 增可选对象），render footer 从 could-not-save 恢复为官方 Full sorted result/grep result stored at 恢复定位 + 检索提示；保存失败/无组合保持诚实 fallback；4 新测 ；**p** F12 shelllocal 时限容差——spawnLeadTime（1s：取消前进程必然在跑）与 killSettleTime（60s：负载下树杀完成界）两常量替换四处裸时序断言（250ms/300ms 取消提前量与两处 20s 完成界），满载 flake 窗口消除 ；**r** sessionreference 深接线——catalog 行 @deepseek-ai/dsh-session-reference 落地（处置表 T3 摘除）：SnapshotReader 适配器（sessionquery 引擎活优先语料→surface 快照投影 user/assistant 事件 + 语料 listing 候选）+ pre-step 全局监听经 Resolver.PrepareDirectMessages 把规范 session 提及改写为持久 untrusted snapshot 上下文（提及文本清除、快照紧随其后、失败降级 next 不死锁回合）+ 标题 labeler 经 session-title；**接线纪律发现入账**：ctx.Inject 待处理回调内的 ctx.Effect 生命周期早于运行期（监听器注册即被撤销）——长生命周期总线监听必须经 PluginSpec.Inject 延迟行 Apply 后在行 ctx 上 Effect 注册（agent-loop SetFactory 同型可证存活）；driver pre-step base 修正为 enter(payload.Messages)（监听者改写此前被循环自身丢弃——契约修复）+ 1 组合级测试 ；**s** zstd 压缩变体落地——jsonl 后端 CompressionZstd：scanZstdFrames 结构扫描逐字移植（帧魔数/描述符/块遍历/CRC，EOF 撕裂帧定位）+ klauspost EncodeAll/DecodeAll 拼接帧编解码（CRC 校验帧）；Store Create/Load/Append/List 及 Backend LoadStored/ReadStoredRaw/readFirstLineHeader/EnsureCurrent 全路径 codec 感知；撕裂帧=截断至最后完整帧边界、写后缓冲层重刷丢失批次；EnsureCurrent 对 zstd 容器短路（已是当前写入编码）+ 3 新测（往返/撕裂修复/非 zstd 容器拒绝）；**s 段后补** gofmt 3 文件（assistant_stream/service/filestore）——r139 审计修复轮收尾：审计发现全部闭环，剩余三项（sandbox ACL 执法器/插件 ABI 受管子进程/PTC run_code 执行基底）为 ROADMAP 既有独立 epic（非本轮审计发现），各需专轮 ；**t** run_code 工具注册——code-runtime-goja 行（gojaruntime 引擎 + tools.DefineTool 注册 run_code 工具，code 参数执行 JS 并返回 completion value，错误分类为 FailureException/Timeout/Abort）；**u** isSeeded 恒写 + 混代目录缺口入账（切压缩配置后旧日志孤儿化，per-file zstd 探测留后续轮）+ session-reference 取消传播入账（ctx.Background()，信号未接通）；**v** sandboxlocal icacls 实验回退（复审四轮实证：父目录 deny 为全系统效果+kill 残留 ACE+叙述三处矛盾——安全组件不合格），Windows 行改 fail-closed（Windows/非 Windows 统一 SANDBOX_UNAVAILABLE，等待 CreateRestrictedToken 专轮）；包注释/catalog 注释/ROADMAP 三处按现实改写；3 测改 fail-closed 契约断言 + pluginhost 插件托管（subprocess.Local spawn+stdio LineTransport+initialize 握手+DiscoveredContributions；3 测含真实 JSON-RPC 往返）| ✅ |
...
1. headless E2E 遗留：流中段 context canceled（诊断线索已收集：非 Do/fail/watchdog/driver-cancel——疑似同步 Apply 阻塞组合与驱动生命周期交互，需异步 run + 生命周期专轮）；session-reference 行（pre-step mention→reference 深度接线）；dsh-host-directory-picker-auto（网关已答 unavailable，双 client 面已从 boot 组合跳过）；**web 聊天回路（r139 审计新增，web 面 #1 缺口）**：session.prompt 提交端点 + cancel/selectModel/search/fork/updateQueue/openWorkspacePath + 文件上传 wire（`fileUploads.upload` + `/api/session/uploadFileBinary`）+ 命令 UI（`commands.list/execute` + commands/change 上总线）+ interaction `$events/result` 闭环（结果注回 approval/user-questions waterfall + 类型化载荷 JSON 投影）+ `fileReferences.list` Remote + follow/page projection baseline 出口（turnOutline/sessionStats 已折叠未上 wire）与 session/list title。
